...
async function runSplitArm() {
  ...
  try {
    ...
    if ((await post('set_page_meta', { path: 'FM Page.ad', tags: 'alpha,beta', aliases: '别名甲' })) !== 'ok') throw new Error('setup set_page_meta FM failed')
    ...
    if ((await post('set_page_meta', { path: 'wiki/CAP 定理.ad', tags: '', aliases: '帽子定理' })) !== 'ok') throw new Error('setup alias CAP failed')
    ...
    if ((await post('set_page_meta', { path: '烟雾警报.ad', tags: '', aliases: '烟感' })) !== 'ok') throw new Error('setup alias 烟雾警报 failed')
